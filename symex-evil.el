...
(defvar symex--evil-keyspec
  '(("h" . symex-go-down)
    ...
    ("C-n" . symex-soar-forward)
    ("C-e" . symex-soar-backward)
    ("C-S-n" . symex-leap-forward) ;; WHY DOES THIS NOT WORK????????
    ("C-S-e" . symex-leap-backward)
    ("C-i" . symex-climb-branch)
    ("C-h" . symex-descend-branch)
    ("p" . symex-paste-after)
    ("y" . symex-yank)
    ...
    ("H" . paredit-raise-sexp)		   ; revisit kb
    ;("C-S-e" . symex-emit-backward)	   ; TODO convert these to colemak
    ("C-(" . symex-capture-backward)
    ;; ("C-S-e" . symex-capture-backward)
    ;; ("C-{" . symex-emit-backward)
    ;; ("C-S-n" . symex-capture-forward)
    ;; ("C-}" . symex-emit-forward)
    ;; ("C-S-e" . symex-emit-forward)
    ("C-)" . symex-capture-forward)
    ("z" . symex-swallow)
    ("Z" . symex-swallow-tail)
    ("R" . symex-evaluate)
    ("M-E" . symex-evaluate-pretty)
    ("d" . symex-evaluate-definition)
    ("C-M-e" . symex-eval-recursive)
    ("T" . symex-evaluate-thunk) ; switch these to be next/prev buffer
	(":" . evil-ex)
    ("t" . symex-switch-to-scratch-buffer)
    ("M" . symex-switch-to-messages-buffer)
    ("r" . symex-repl)
										;("R" . symex-run) ; not sure what these do
    ("X" . symex-run)
    ("|" . lispy-split)
    ("m" . symex-join)
    ("\\" . symex-splice)
	...
	("u" . symex-insert-at-beginning)
    ("U" . symex-insert-before)
    ("W" . symex-wrap)
